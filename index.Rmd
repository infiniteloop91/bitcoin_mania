...
![A photo](https://github.com/infiniteloop91/bitcoin_mania/blob/master/flowers.jpg?raw=true)
...
# II. Data 
  ...
  The volume of stock trades per day was pulled from the New York Stock Exchage's public data set. The data is specifically from the Volume Summary. This set provides a count of total trades, volume, and dollar volume by market center (exchange) in each NYSE-traded issue. The only metric used for the analysis was totle trades. 
  The volume of Bitcoin trades was acquired from data.bitcoinity.org. This organization keeps track of hundreds of different blockchain metrics, but the trade volume was used for the project. The data set breaksdown trade volume by individual coin type or brand, but the sum of all the various coins was used for the analysis. 
  The price of bitcoin over time was taken directly from bitcoin.com. This only takes into account the price of the main cryptocurrency Bitcoin and does not reflect the price of the various alt coins offered on the market. 
  The US GDP data was gathered from tradingeconomics.com. Trading Economics is a company that provides users access to over 20 million economic indicators. 
...
In this project for the first set, the dependent variable was the number of bitcoins traded and the independent variable was the volume of Google searches for the word “bitcoin”. For the second series of tests, the dependent value was the number of equity securities traded and the independent variable was the number of Google searches for the word “stocks”. 

These tests were run again with a second independent or predictor variable. For this series, US GDP was factored in, since it is an indicator for the health of the economy. This value was transformed with a log function in order to measure the percent change of the US GDP. 

The final series of test was conducted using a Granger Causality test. Professor Sir Clive Granger developed this test in the 1960s. It is based on the premise that if a signal X1 "Granger-causes" (or "G-causes") a signal X2, then past values of X1 should contain information that helps predict X2 above and beyond the information contained in past values of X2 alone. 
...
The Granger Causality test for Bitcoin searches and the volume of Bitcoin trades produced an -0.5935 F-test value and a p-value of .4458. Given this evidence and the high p-value we cannot reject the null hypothesis that Google Search trends do not help predict future trade volume for bitcoin. 

The Granger test was run on the Google search trends for stocks and the volume of trades of equity securities. The test produced a .0197 F-test value and a .8892 p-value. So, again we cannot reject the null hypothesis that Google Search trends do help help predict future trade volume for securities. 
