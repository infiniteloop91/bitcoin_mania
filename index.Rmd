...
Each year, seven million tulips bloom in Keukhenf, a public garden located in Southern Holland. Beside windmills, bicycles and chocolate, the small flower is one of the most iconic symbols associated with the Netherlands. While the tulip is known for its colorful petals and delicate structure, it decimated the nation’s economy at the beginning of the 17th century.	
...
```
<br>
While anyone may buy or sell securities with the rise of online banking and mobile applications such as Robinhood and Acorn, most people still have a professional stock broker or financial advisor manage their money. There is no intermediary with Bitcoin apps like Coinbase, Kraken, and Gemini. The question then arises: do people hear about Bitcoin while talking to a friend, watching television, or listening to a podcast and then use Google to find out where they can purchase or trade the crypto currency. Based on this question or theory it is hypothesized that there is a stronger correlation between Google search trends for and the volume of Bitcoin transactions than the volume of traditional equity securities transactions. The null hypothesis would be that the associations/correlations are equivalent.


In 2008, researchers from Google began to use search data to attempt to predict the overall prevalence of the flu in given geographic areas. The model was incorrect for 100 out of the first 108 weeks following its initial launch. The main issue with the model was that the model was susceptible to false positives that included users searching for topics related to flu-like symptoms, but were not actual cases of the flu. The second issue was that the model could not deal with the time lag between when a person searches for flu related symptoms and is actually infected with the flu (Salzberg, 2014).
	
	
The Google Flu Trends project was released to the public and the results were published in the journal Nature in 2014. It was one of the first large scale research efforts that leverages publically available search data to predict future behavior. Melody Y. Huang and other members of the University of California Los Angeles’ statistics department used leveraged this search trend data in attempt to predict movements of the S&P 500 in the paper Forecasting Stock Market Movements using Google Trend Searches. While the team did not find significant results and could not produce positive returns using financial backtests, they did examine the pros and cons of using various predictive models as a mechanism for stock prediction using search data. 


# II. Data 
The data for the research came from a variety of sources. The independent variables came from Google Search Trends. Google Search Trends is a tool used for people to see the demand or coverage of a given topic. Google released the product in 2004. The tool was expanded to allow users to compare search trends for multiple phrases as well as limit the search to individuals in a given geographic area. The phrases are case-sensitive. The two used for this project were "stocks" and "Bitcoin". The case-sensitive nature to these results is also one of the main drawbacks. If an individual is searching for stocks to purchase or how to buy Bitcoin, there are thousands of different variations that could be typed out. This also rules out individuals who do not use Google as a medium to get to a marketplace to buy/sell securities or Bitcoin.


The volume of stock trades per day was pulled from the New York Stock Exchange’s public data set. The data is specifically from the Volume Summary. This set provides a count of total trades, volume, and dollar volume by market center (exchange) in each NYSE-traded issue. The only metric used for the analysis was total trades. 


The volume of Bitcoin trades was acquired from data.bitcoinity.org. This organization keeps track of hundreds of different block chain metrics, but the trade volume was used for the project. The data set breaks down trade volume by individual coin type or brand, but the sum of all the various coins was used for the analysis.  The price of Bitcoin over time was taken directly from bitcoin.com. This only takes into account the price of the main crypto currency Bitcoin and does not reflect the price of the various alt coins offered on the market.  The US GDP data was gathered from tradingeconomics.com. Trading Economics is a company that provides users access to over 20 million economic indicators. 
 

# III. Methodology
The first test to examine the relationship or association between the dependent variables of securities traded and Bitcoin products traded and the independent variables of the number of Google searches related to the two was a basic correlation. Correlations range from -1 to 1. They measure the strength and linear direction of two variables. 
...
These tests were run again with a second independent or predictor variable. For this series, US GDP was factored in, since it is an indicator for the health of the economy. This value was transformed with a log function in order to measure the percent change of the US GDP. It was input into both linear models as an interaction variable. The reasoning behind this was that GDP as a proxy for economic health may influence if people are searching for stocks to buy when the economy is good. The counter would be that Bitcoin and security trades would increase if the economy was bad and people wanted to take their money out of the market. Linear models are limited in analyzing time series data based on the fact that they cannot factor in a temporal lag. Researchers could shift the data frames to input a lag, but this can also be done using a Granger Causality test. 
Professor Sir Clive Granger developed this test in the 1960s. It is based on the premise that if a signal X causes a signal Y, past values of X should contain information that predicts Y more accurately than just the information contained in past Y values. 
...
# V. Results
Both correlations produced negative correlations with minimal effects. The correlation coefficient for Google Search Trends and the number of Bitcoin trades was -.28. The correlation coefficient for Google Search Trends and the volume of equity securities traded was  -.24. The hypothesis was based on the premise that there was a positive relationship between search trends and the volume of trades for both Bitcoin and equity securities, but this was proven to be false. These findings suggest that as more Google searches go up, trading volume goes down. This could be attributed to a temporal gap between when individuals search the phrases “stocks” or “Bitcoin”, but the lag concept will be tested with the Granger test. 
...
Again this model was adjusted and the log of GDP was added as an interaction variable. This model did produce a significant p-value of .045. Similar to the first model, when GDP was factored in the number of searches because a positive factor based on the coefficient of 1998.06. When GDP interacted with the number of searches though, the effect was negative on the volume of transactions. The fact that GDP has a negative relationship with the volume of equity contradicts the Disposition Effect. The Disposition Effect states that people sell stocks that earn money and hold onto losing stocks hoping that they will turnaround and recoup some loses. If US GDP increases companies should theoretically earning profits that should theoretically increase the value of their stocks and cause selling to increase. This relationship may suggest that people hold on to stocks when the economy is doing well. 
...
# VI. Conclusion
Based on the analysis, the hypothesis that there is a stronger correlation between Google search trends for and the volume of Bitcoin transactions than the volume of traditional equity securities transactions can be rejected. There is little association between search data and the number of transactions. Given the evidence provided by both regression models, there is a negative relationship between the number of Google searches and transactions. The data is limited by the fact it is case sensitive and there are millions of different variations individuals could search for Bitcoin or stock related topics. Also, both the equity and Bitcoin market are greatly influenced by bot and high-frequency-trading activity. As seen in the flash crash in the 2000s, these two factors play a large role in the volume of transactions. This model is an extremely simplified version of market factors. Markets are interdependent on nearly an infinite number of parameters –some of which cannot be measured quantitatively. The use of search data for predicting behavior may be extremely useful in more isolated or more target spaces with more detailed models. While the results were inconclusive, they can be used built off of or as a reference point for future research. 
...
"Tulips and Flower Bulbs in the Netherlands." Visit the Beautiful Cities of the Dutch Golden Age - Holland.com.      June 04, 2018. Accessed December 15, 2018. https://www.holland
